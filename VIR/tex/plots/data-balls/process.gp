...
set title "Fallverlauf der Stahlkugeln"
...
do for [t=1:9] {
	...
	m = 1; c = 1;
	fit f(x) inp u 1:($2 * 1000):(5) yerrors via m,c  # Change to mm
	A_m[t] = m
	A_m_err[t] = m_err
	A_c[t] = c
	A_c_err[t] = c_err
...
set key right vertical maxrows 10 width -7

plot for [i=1:9] input_mp[i] u 1:($2 * 1000):(5) with yerrorbars title titel_mp[i] pointtype 77 lc i, for [i=1:9] A_m[i]*x+A_c[i] title titel[i] lc i
...
# LaTeX table output
print "\\toprule"
print "Kugel Nr. & $m/\\si{\\milli\\meter\\per\\second}$ & $c/\\si{\\milli\\meter}$ & $\\chi^2_\\text{red}$ \\\\"
print "\\midrule"
do for [t=1:9] {
